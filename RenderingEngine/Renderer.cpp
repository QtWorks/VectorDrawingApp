...
void Renderer::save(const char *filename)
{
	...
	if(f == NULL) MessageBox(NULL, L"Cannot Create File", L"Cannot Create File", MB_OK);
...
void Renderer::load_0_0_1(FILE *stream)
{
	...
	for(int i=0; i<shapeCount; i++)
	{
		...
		if(objectType == Renderer::OBJECT_TYPE_BRUSH) 			shape = new BrushObject();
		else if(objectType == Renderer::OBJECT_TYPE_ELLIPSE) 	shape = new EllipseObject();
		else if(objectType == Renderer::OBJECT_TYPE_LINE) 		shape = new LineObject();
		else if(objectType == Renderer::OBJECT_TYPE_RECTANGLE) 	shape = new RectObject();
...
bool Renderer::processSelectOperation(int mouseState, int x, int y)
{
	...
	if(m_objects.size() <= 0) return false;
		...
		if(selecting)
		{
			...
			for(unsigned int i=0; i<m_objects.size(); i++)
			{
				...
				else
				{
					...
					{
						IElement *s = m_selectedShapes[k];
						if(s == shape) m_selectedShapes.erase(m_selectedShapes.begin()+k);
					}
				}
			}
			...
			if(selectionRectGeometry != NULL) selectionRectGeometry->Release();
		}
		else if(moveObjects)
		{
			int relX, relY;
